...
while our discriminator will just consist of 3 convolutional layers just like in proposed model from Brunner et. al. \cite{brunner2018symbolic}
...
This will ensure that input and output having mutual information, and help regularize and retain information from the input data.
\par
During training, we made sure that our model to have balanced power of generator and discriminator, so that the model can be stable.
While our generator is trying to learn different characteristics of different genre,
it could be very easy for our generator to fool our discriminator. To solve this problem we tried to add two extra discriminator
and trained to classify the fake data from different genre instead of having one target genre.
This will eventually help our model retain the input data's structure, which will make our output sound similar to the input data.
